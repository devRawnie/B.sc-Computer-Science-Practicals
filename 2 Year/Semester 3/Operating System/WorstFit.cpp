...
void assignMemory()
{
	int t_req = 0, t_size = 0, max = MAXINT, comp = 0, pos;
	for(i=0;i<n_process;i++)
	{
		...
		max = MAXINT ;
		pos = -1;
		for(j=0;j<n_blocks;j++)	//this loop finds the block no with maximum wastage after allocating process i
		{
...
//			cout<<"\nin j";
//			cout<<"\ncomp "<<j<<" : "<<comp;
			if(comp == 0)		//if the block size is equal to memory req
			{
	//			cout<<"\n in j if 1";
				max = comp;
				pos = j;
